...
- Incredibly dynamic system
    ...
    + Some exist only within cells $\implies$ some are exported
...
- By dogma, Nucleic Acids don't exist outside cells (apart from when they do)
    + tRNA-mRNA molecules are extracellular signalling molecules in plants
:::
...
- lncRNAs form highly complex structures <br>$\implies$can silence chromosomal regions (e.g. *XIST*)[^1]

::::

:::: {.fragment fragment-index=3}

- rRNAs are key components of ribosomes

::::

:::: {.fragment fragment-index=4}

- tRNAs interact with ribosomes <br>$\implies$ translate mRNA into proteins[^2]
...
- tRNAs are generally not interesting either (expect when forming dicistronic tRNA-mRNA)
:::
...
2. Alignment to a reference transcriptome
    ...
    - Reads align to multiple transcripts $\implies$ uncertainty
...
- Includes 517 extra sequences (scaffolds, patches, haplotypes)
    ...
    + Some genes have transcripts defined on scaffolds not chromsomes
...
- A reference pan-genome is now available[^3]
...
- Fit standard statistical models
...
- Sequencing generally done in batches of $\leq$ 96 $\implies$ batch effects
    ...
    + Llead to <span style="color:red;">&#8679;</span> false discoveries <span style="color:red">&#8681;</span> true discoveries
...
- For simple comparisons we often assume effect size (change in expression) is only relevant if large enough
    ...
    + We will find correlations between expression patterns and predictor variables $\implies$ doesn't elucisdate mechanism
...
::: {.column}
![Example MA plot](https://github.com/DRMCRL/MDA-MB-453_DHT-RNASeq/blob/master/docs/figure/dge_analysis.Rmd/plotMA-1.png?raw=true)
:::

::: {.column}
![Example Volcano plot](https://github.com/DRMCRL/MDA-MB-453_DHT-RNASeq/blob/master/docs/figure/dge_analysis.Rmd/plotVolcano-1.png?raw=true){width="80%"}
:::
...
- Look for biologically relevant signals in the set of DE genes
    + Enriched pathways, common transcription factors etc
    + Drug target signals
- Compare to public datasets (if appropriate)
